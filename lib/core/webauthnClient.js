...
class WebauthnHardwareClient {
    ...
    async registerSign(origin, username, displayName, payload, emitPublicKey, isMobile, keepSigning = true) {
        try {
            const credentialCreationOptions = await this.server.registrationOptions(isMobile, username, payload);
            ...
            const registerResponse = await this.server.signData(origin, credential, challenge, (0, utils_1.arrayify)(ethers_1.ethers.utils.sha256(payload)), credentialCreationOptions.user.id, emitPublicKey);
