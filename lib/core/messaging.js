...
class MessagingService {
    ...
    async createTopic(topic, options, blockPublisher) {
        let pub = blockPublisher;
        if (options.middleware && options.middleware.outgoing) {
            pub = blockPublisher.pipe(rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), rxjs_1.tap(), ...options.middleware.outgoing);
        }
        ...
        this.waku.relay.addObserver((msg) => {
            ...
            if (msg.contentTopic === topic) {
                const isValidMessage = this.validatePublicKeyMessage('elsegundo', msg.publicKeyMessage);
        ...
        const cancel = pub.subscribe(async (block) => {
            let message = { payload: block.document };
            if (this.pubkey && this.defaultAddress && this.web3Provider) {
                const sig = await this.signEncryptionKey('elsegundo', this.pubkey, this.defaultAddress, this.web3Provider.provider.send);
                const pubkeyMessage = {
                    signature: sig,
