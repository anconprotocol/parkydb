...
const main_1 = require("js-waku/build/main");
const rxjs_1 = require("rxjs");
class MessagingService {
    constructor() { }
    async load(key, data) { }
    async bootstrap() {
        this.waku = await main_1.Waku.create({ bootstrap: { default: true } });
    ...
    async createTopic(topic, blockPublisher) {
        ...
        const cancel = blockPublisher.subscribe(async (block) => {
            const msg = await main_1.WakuMessage.fromBytes(block.dag.bytes, topic);
    ...
    async createChannel(topic, options, blockPublisher) {
        ...
        const cancel = blockPublisher
            ...
            console.log(view);
            const msg = await main_1.WakuMessage.fromBytes(view, topic, {
