...
class ParkyDB {
    ...
    async createTopicPubsub(topic, options) {
        const privateKey = js_waku_1.generatePrivateKey();
        const pubkey = js_waku_1.getPublicKey(privateKey);
        ...
        return this.messagingService.createTopic(topic, options, options.sigkey || utils_1.hexlify(privateKey), options.encryptionPubKey || utils_1.hexlify(pubkey));
    }
    async getWallet() {
