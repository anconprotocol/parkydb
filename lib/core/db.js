...
class ParkyDB {
    ...
    async initialize(options = { withWallet: {}, wakuconnect: null }) {
        ...
        if (options.withWeb3) {
            this.messagingService = new messaging_1.MessagingService(options.withWeb3.provider, options.withWeb3.pubkey, options.withWeb3.defaultAddress, options.withWeb3.pubkeySig);
        }
    ...
    async createTopicPubsub(topic, options) {
        return this.messagingService.createTopic(topic, options, this.onBlockCreated);
    }
    async getWallet() {
