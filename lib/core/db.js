...
class ParkyDB {
    ...
    async emitKeyExchangePublicKey(topic, options) {
        ...
        const obs = pubsub.onBlockReply$.pipe(rxjs_1.map(() => {
            pubsub.publish({
        ...
        return rxjs_1.lastValueFrom(obs);
    }
    async subscribeKeyExchangePublicKey(topic, options) {
        try {
            options.canSubscribe = true;
            options.isKeyExchangeChannel = true;
            const pubsub = await this.messagingService.createTopic(topic, options);
            pubsub.publish({ askForEncryptionKey: true });
            const config = await rxjs_1.lastValueFrom(pubsub.onBlockReply$);
